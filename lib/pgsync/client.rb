  ...
  class Client
    ...
    def sync(args, opts)
      ...
      if opts[:list]
        ...
        puts "-------------------------------------"
        puts "Normal PGSync Sync Tables:  pgsync --preserve --in-batches #{sync_tables.sort_by(&:last).map(&:first).join(",")}"
        puts "-------------------------------------"
        ...
        puts "-------------------------------------"
        puts "Truncate & Sync Tables NO PK : pgsync --truncate #{full_sync_tables_no_pk.sort_by(&:last).map(&:first).join(",")}"    
